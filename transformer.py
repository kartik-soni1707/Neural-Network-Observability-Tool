...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, model_dim: int, num_heads: int):
        ...
        self.multi_head_attention=self.MultiHeadedSelfAttention(model_dim,num_heads)
        self.ff=self.VanillaNeuralNetwork(model_dim)
        self.layer_nomr_1=nn.LayerNorm(model_dim)
        self.layer_nomr_2=nn.LayerNorm(model_dim)
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        x=embedded+self.multi_head_attention(self.layer_nomr_1(embedded))
        x+=self.ff(self.layer_nomr_2(x))
        return x
